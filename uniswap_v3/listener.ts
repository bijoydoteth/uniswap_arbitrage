...
import { calcProfitExternal, filterUniqueLastLog, findBestProfit, getSpotPrice, optimizeV3 } from "../helpers";
...
async function uniV3Events() {
  ...
  providerws.on("block", async (blockNumber: any) => {
    ...
    const decode = filterlogs.map(async log=>{
      try{
          const decodelog = iface.parseLog(log);

          const pairAddress = log.address;
          const lp_pool = new LpPool({ address: pairAddress });
          await lp_pool.init();

          // Check DB and add new pool
          lp_pool.checkDB()
          ...
          const grossprofit_threshold = 0.0001
